...
- A formal accessibility audit of this project found multiple WCAG violations
  across severity levels. Key patterns: non-descriptive alt text (filenames +
  "image"), missing language attributes on non-English content, text truncation
  at 200% zoom, auto-dismissing error messages, ARIA over-use. Fixes should
  reference specific WCAG criteria from the audit.
...
- Test with macOS Voice Control in addition to screen readers. The project's
  accessibility audit found Voice Control could not operate header navigation
  or certain engagement features.
