...
- The project's accessibility audit found that ARIA attributes were used where
  standard HTML elements would have worked. Do not add `aria-live` or
  `aria-atomic` without explicit justification. From the project's planning
  notes: the codebase had accumulated excessive aria-live regions that needed
  to be removed rather than extended.
...
- Add `aria-live` regions for content that already has semantic HTML equivalents
