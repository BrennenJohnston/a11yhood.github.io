...
Core principle: Build quality infrastructure first, not product. AI agents
should be directed at building quality infrastructure -- tests, linting,
accessibility checks, CI configuration -- not at generating more features. The
agents that test code must constrain the agents that write code.
...
- Domain-specific content where false positives create harm (e.g., alt text for
  assistive technology products, safety-critical documentation). AI-generated
  alt text for AT products risks producing inaccurate descriptions that create
  new accessibility barriers. Use human-in-the-loop community process instead.
- 3D models and spatial reasoning. AI cannot reliably generate or manipulate 3D
  models. The project's planning notes document significant time lost to AI 3D
  tasks. STL-to-SCAD conversion is non-reversible. OpenSCAD parametric code
  generation requires human design.
- Tag provenance and verification decisions
