...
The software development cycle is circular: build -> test -> fix -> refactor ->
build. The recurring problem in AI-assisted projects is always widening without
narrowing. AI should be used in the narrowing phases.

The Ouroboros has no endpoint — it is a continuous loop. Quality is not a
destination but a rhythm maintained by alternating between construction and
consolidation. From the project's planning notes: the team did not yet have
enough parts of the cycle in place to close the loop.

### PR-driven demonstration

Guardrails should be visible in code (tests, CI checks, linting rules), not
just in documents. Small, demonstrative PRs over planning discussions. From the
project's planning notes: the team recognized a gap between what they were
planning to do and what was actually being done in code.
