...
## OQ-1: Pixi enforcement (status: in progress)

Pixi adoption PRs are open in the a11yhood repositories.
...
### Third option: encourage upstreams to improve metadata

The project's planning notes surfaced a third strategy: rather than generating
alt text locally, invest effort in encouraging upstream AT product publishers to
improve their own metadata (descriptions, images, categorization). This shifts
the responsibility to the source of truth and avoids the false-positive risk
entirely.
...
## OQ-7: HTMX alternative frontend

### Should the project evaluate HTMX + HTML5 as a React replacement?

The project's planning notes propose HTMX + server-rendered HTML as an
alternative to the current React SPA. HTMX returns HTML fragments from the
server, eliminating the need for a JavaScript framework for most interactions.
This aligns with the "semantic HTML first" principle and could simplify the
accessibility story.

| Option | Trade-off |
| --- | --- |
| **Evaluate HTMX** | Dramatically simpler frontend; requires backend rendering; loses React ecosystem (Radix UI, TanStack Query); significant migration cost |
| **Keep React** | No migration cost; mature ecosystem; higher frontend complexity |
| **Build both as an exercise** | See OQ-9; team learns by comparison; doubles short-term effort |

**If no decision is made:** The project continues with React. HTMX remains a
future exploration item.

---

## OQ-8: Tag governance

### Who maintains the constellation tagging system?

The constellation tagging system (ISO 9999 mapping, community tags, tag
relationships) is the most-discussed feature but has no governance model.

| Decision | Options |
| --- | --- |
| **Who maintains the ISO 9999 mapping?** | Lead developer only / designated tag maintainer / community with review |
| **Who resolves tag conflicts?** | BDFL decision / community vote / automated merge rules |
| **How do community vs. admin tags interact?** | Community tags are suggestions; admin tags are canonical / Both are equal / Community tags feed into admin review queue |

**If no decision is made:** The playbook ships with `[CONFIGURE: ...]`
placeholders for tag governance. Tag-related features cannot safely proceed
without this decision.

---

## OQ-9: "Everyone builds their own frontend" exercise

### Should each team member build their own frontend?

The project's planning notes propose that each team member builds their own
frontend implementation as a learning exercise. This would demonstrate the
team's diverse approaches to the same problem and reveal assumptions about what
the "right" frontend looks like.

| Option | Trade-off |
| --- | --- |
| **Pursue the exercise** | Deep learning for each team member; reveals architectural assumptions; time-intensive |
| **Skip it** | No time investment; team continues with shared React frontend |
| **Time-box it** | Each person gets 2 hours; compare results in a team session |

**If no decision is made:** The team continues with the shared React frontend.
The exercise remains an option for a future team session.
