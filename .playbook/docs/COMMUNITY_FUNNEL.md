...
### AT contributor hackathon (example)

A hackathon format for assistive technology users completing "good first issues"
in an AT project. This format pairs domain-expert contributors with scoped,
well-tested tasks and produces both code and accessibility testing insights.

### Structured lab session (example)

Attention-span-aware pacing, structured presentation + unstructured lab
activities, reproducible format captured on video. Regular structured lab
sessions create a cadence of contributor engagement and generate shareable
content that demonstrates the project's values in action.
