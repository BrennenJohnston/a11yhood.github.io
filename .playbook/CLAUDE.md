...
11. Treat this codebase as legacy. Treat AI-generated code as legacy code from birth.
    Wrap in tests before modifying. Prefer refactoring to rewriting. Avoid
    "replace" framing.
...
Task descriptions help AI agents understand purpose. `inputs`/`outputs` enable
caching. `depends-on` chains tasks.
...
Development alternates: narrow (test, refactor, remove) then widen (build). AI
should be directed at narrowing phases. Map to BUILD / REFACTOR / STABILIZE in
`PROCESS_CADENCE.md`.
...
## AI-appropriate vs AI-inappropriate tasks

Appropriate: boilerplate, pattern replication (after gold standard), test
implementations (from human specs), documentation drafts, quality infrastructure,
formatting.

Inappropriate: architecture decisions, first implementations, design decisions,
alt text for AT products, 3D models/spatial reasoning, user-facing text requiring
domain knowledge, tag provenance/verification decisions.
