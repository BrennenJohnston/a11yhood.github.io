...
| 7 | Task serves safeguards, not features | Verify the task builds quality infrastructure (tests, linting, a11y checks) OR is a well-scoped feature with all safeguards already in place. Quality infrastructure first; features second. | Warning |
...
- [ ] Task serves safeguards, not features (quality infrastructure first)
```

### Project-specific configuration

- **AI-appropriate tasks for this project:** `[CONFIGURE: e.g., boilerplate, repetitive patterns, test implementations (from human specs), documentation drafts, quality infrastructure (CI configs, linting rules, accessibility checks), formatting]`
- **AI-inappropriate tasks for this project:** `[CONFIGURE: e.g., architecture decisions, first implementations of new patterns, design decisions, alt text for AT products, 3D models/spatial reasoning, user-facing text requiring domain knowledge, tag provenance/verification decisions]`
