...
| 9 | Voice Control tested | If header/nav/form elements changed, verify macOS Voice Control can operate them. | Warning |
| 10 | Error messages persistent and announced | Verify error messages do not auto-dismiss, do not overlap, and are announced to screen readers. | Yes |
...
| 9 | Complexity delta is neutral or negative | Check lines added vs removed. PRs that add complexity without removing equivalent complexity require justification. | Warning |
...
- [ ] Voice Control tested (if header/nav/form changed)
- [ ] Error messages are persistent and announced
...
- [ ] Complexity delta is neutral or negative (or justified)
```
